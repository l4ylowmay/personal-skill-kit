...
1. **冻结题位。** 默认100题、90分钟、100分，模块20/20/15/30/15，易中难28/67/5；这是名为 `sz-historical-modal-v1` 的历史仿真基准，不是当前官方大纲。二级配额及每题长度、计算强度、材料组见内置蓝图。复制到本次 `work/blueprint.json` 并记录哈希，不能只记总题量。
2. **编写新题。** 阅读 [题目质量](references/question-quality.md) 与 [数据契约](references/data-contract.md)。每题关联题位，按材料负担、设问和干扰机制原创；不要只换旧题数字。模板是底层解法，不是唯一指定题面。地方/时政的实际事实在本次命题时核验，不继承2026年的事实有效性。
3. **独立复核。** 导出不含答案解析的视图，重新作答后再比较；所有计算另写独立求解脚本，逻辑按适用情形枚举，事实逐项核对，选项逐项排查双解。运行结构及质量记录检查，修正错误后复验。参考 [校准与复核契约](references/calibration-and-review.md)。不能把填写“通过”或脚本通过当作答案正确的证明。
